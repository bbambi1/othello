...
BitBoard BitBoard::rot90() const {
  return {flipDiagonal(flipHorizontal(blackBoard)),
          flipDiagonal(flipHorizontal(whiteBoard))};
}

BitBoard BitBoard::rot180() const {
  return {flipVertical(flipHorizontal(blackBoard)),
          flipVertical(flipHorizontal(whiteBoard))};
}

BitBoard BitBoard::rot270() const {
  return {flipAntiDiagonal(flipHorizontal(blackBoard)),
          flipAntiDiagonal(flipHorizontal(whiteBoard))};
}

BitBoard BitBoard::mirrorHorizontal() const {
  return {flipHorizontal(blackBoard), flipHorizontal(whiteBoard)};
}

BitBoard BitBoard::mirrorVertical() const {
  return {flipVertical(blackBoard), flipVertical(whiteBoard)};
}

BitBoard BitBoard::mirrorDiag() const {
  return {flipDiagonal(blackBoard), flipDiagonal(whiteBoard)};
}

BitBoard BitBoard::mirrorAntiDiag() const {
  return {flipAntiDiagonal(blackBoard), flipAntiDiagonal(whiteBoard)};
}
...
uint64_t BitBoard::getStableMaskForPlayerBitwise(bool isBlack) const {
  ...
  uint64_t stable = 0;
  if (player & positionToMask(0, 0))
    stable |= positionToMask(0, 0);
  if (player & positionToMask(0, 7))
    stable |= positionToMask(0, 7);
  if (player & positionToMask(7, 0))
    stable |= positionToMask(7, 0);
  if (player & positionToMask(7, 7))
    stable |= positionToMask(7, 7);
  ...
  while (stable != prev) {
    ...
    uint64_t surrounded = north & south & east & west & ne & nw & se & sw;
...
static constexpr uint64_t cornerMask = (1ULL << 0) |  // (0,0)
                                       (1ULL << 7) |  // (0,7)
                                       (1ULL << 56) | // (7,0)
                                       (1ULL << 63);  // (7,7)
