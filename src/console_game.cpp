...
void ConsoleGame::selectGameMode() {
    ...
    if (currentMode == ConsoleGameMode::HUMAN_VS_AI || currentMode == ConsoleGameMode::AI_VS_AI) {
        selectAIAgent();
    }
...
void ConsoleGame::selectAIAgent() {
    ...
    std::cout << "AI Agent set to: " << getAIAgentTypeString(selectedAIAgent) << std::endl;
}
...
void ConsoleGame::selectOpponentAIAgent() {
    ...
    std::cout << "Opponent AI Agent set to: " << getAIAgentTypeString(selectedOpponentAIAgent) << std::endl;
}
...
void ConsoleGame::configureAISettings() {
    ...
    // Time limit
    std::cout << "Current AI Time Limit: " << aiTimeLimit.count() << "ms" << std::endl;
    int timeLimit = getIntInput("Enter new time limit in milliseconds (100-10000): ", 100, 10000);
...
void ConsoleGame::startGame() {
    ...
    while (gameRunning) {
        ...
        if (currentMode == ConsoleGameMode::HUMAN_VS_HUMAN || 
            (currentMode == ConsoleGameMode::HUMAN_VS_AI && currentPlayer == CellState::BLACK)) {
            ...
            // AI player's turn
            AIAgentBase* currentAI = (currentPlayer == CellState::BLACK) ? aiAgent.get() : opponentAIAgent.get();
...
void ConsoleGame::initializeGame() {
    ...
    if (currentMode == ConsoleGameMode::AI_VS_AI) {
        opponentAIAgent.reset(createAIAgent(selectedOpponentAIAgent, "Opponent AI"));
    }
}
...
void ConsoleGame::displayScore() {
    ...
    std::cout << "Score - Black: " << blackCount << ", White: " << whiteCount << std::endl;
    std::cout << std::endl;
}
...
void ConsoleGame::displayCurrentPlayer() {
    ...
    if (currentMode == ConsoleGameMode::HUMAN_VS_AI && currentPlayer == CellState::WHITE) {
        std::cout << " (" << getAIAgentTypeString(selectedAIAgent) << ")";
    } else if (currentMode == ConsoleGameMode::AI_VS_AI) {
        AIAgentType aiType = (currentPlayer == CellState::BLACK) ? selectedAIAgent : selectedOpponentAIAgent;
...
void ConsoleGame::displayGameOver() {
    ...
    if (blackCount > whiteCount) {
        std::cout << "Black wins! (" << blackCount << " vs " << whiteCount << ")" << std::endl;
    } else if (whiteCount > blackCount) {
        std::cout << "White wins! (" << whiteCount << " vs " << blackCount << ")" << std::endl;
    } else {
        std::cout << "It's a tie! (" << blackCount << " vs " << whiteCount << ")" << std::endl;
    }
...
void ConsoleGame::displayValidMoves() {
    std::vector<std::pair<int, int>> validMoves = board.getValidMoves(currentPlayer);
    ...
    if (validMoves.empty()) {
        ...
        for (size_t i = 0; i < validMoves.size(); ++i) {
            if (i > 0) std::cout << ", ";
            std::cout << static_cast<char>('A' + validMoves[i].second) << (validMoves[i].first + 1);
        }
        std::cout << std::endl;
    }
    std::cout << std::endl;
    ...
    while (true) {
        ...
        if (input.length() >= 2) {
            ...
            if (colChar >= 'A' && colChar <= 'H' && rowChar >= '1' && rowChar <= '8') {
                ...
                std::cout << "Invalid format. Use format like A1, B2, etc." << std::endl;
            }
        } else {
...
bool ConsoleGame::isValidMove(int row, int col, CellState player) {
    ...
    return std::find(validMoves.begin(), validMoves.end(), std::make_pair(row, col)) != validMoves.end();
}
...
void ConsoleGame::handlePass() {
    passCount++;
    switchPlayer();
    std::cout << getPlayerName((currentPlayer == CellState::BLACK) ? CellState::WHITE : CellState::BLACK) 
              << " passes." << std::endl;
    waitForKeyPress();
}
...
AIAgentBase* ConsoleGame::createAIAgent(AIAgentType type, const std::string& name) {
    switch (type) {
        case AIAgentType::RANDOM:
...
void ConsoleGame::displayHelp() {
    ...
    std::cout << "- Enter moves in format A1, B2, etc. (column letter, row number)" << std::endl;
    std::cout << "- Type 'pass' or 'p' to pass your turn" << std::endl;
    std::cout << "- The game ends when both players pass or the board is full" << std::endl;
    ...
    std::cout << "- Greedy: Always takes the move that flips the most discs" << std::endl;
    std::cout << "- MinMax: Uses minimax algorithm with lookahead" << std::endl;
    std::cout << "- BitBoard: Uses bitboard representation for efficiency" << std::endl;
    std::cout << std::endl;
    waitForKeyPress();
}
