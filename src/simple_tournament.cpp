...
void SimpleTournament::addAgent(const std::string &type,
...
void SimpleTournament::runRoundRobin() {
  ...
  std::cout << "Rounds per matchup: " << config_.roundsPerMatchup
            << " (each round plays both sides for fairness)" << std::endl;
  std::cout << "Time limit per move: " << config_.timeLimit.count() << "ms"
            << std::endl;
  ...
  for (size_t i = 0; i < agents_.size(); ++i) {
    for (size_t j = i + 1; j < agents_.size(); ++j) {
      std::string matchup =
          agents_[i]->getName() + " vs " + agents_[j]->getName();
...
GameResult SimpleTournament::playSingleGame(AIAgentBase *blackAgent,
                                            AIAgentBase *whiteAgent) {
  GameResult result;
    ...
    if (timedOut || crashed) {
      if (currentPlayer == CellState::BLACK) {
        ...
        result.failureReason =
            agentName + " " + (timedOut ? "timed out" : "crashed");
      } else {
...
std::pair<int, int>
SimpleTournament::getSafeMove(AIAgentBase *agent, const Board &board,
...
void SimpleTournament::updateStats(const GameResult &result) {
  ...
  for (auto &[name, stats] : agentStats_) {
    if (stats.gamesPlayed > 0) {
      ...
      stats.averageScore =
          static_cast<double>(stats.totalScore) / stats.gamesPlayed;
      stats.averageScoreAgainst =
          static_cast<double>(stats.totalScoreAgainst) / stats.gamesPlayed;
    }
  }
}

void SimpleTournament::printProgress(int current, int total,
                                     const std::string &currentMatch) const {
  if (!config_.enableVisualFeedback)
    return;
...
void SimpleTournament::printResults() const {
  ...
  std::cout << std::left << std::setw(15) << "Agent" << std::setw(6) << "Games"
            << std::setw(6) << "Wins" << std::setw(6) << "Loss" << std::setw(6)
            << "Draw" << std::setw(8) << "Win%" << std::setw(8) << "AvgScore"
            << std::setw(6) << "TO" << std::setw(6) << "Crash" << std::endl;
  std::cout << std::string(80, '-') << std::endl;

  for (const auto &stats : rankedResults) {
    std::cout << std::left << std::setw(15) << stats.agentName << std::setw(6)
              << stats.gamesPlayed << std::setw(6) << stats.wins << std::setw(6)
              << stats.losses << std::setw(6) << stats.draws << std::setw(8)
              << std::fixed << std::setprecision(1) << (stats.winRate * 100.0)
              << std::setw(8) << std::fixed << std::setprecision(1)
              << stats.averageScore << std::setw(6) << stats.timeouts
              << std::setw(6) << stats.crashes << std::endl;
  }
...
void SimpleTournament::saveResults(const std::string &filename) const {
  ...
  file << "Final Rankings:\n";
  file << std::left << std::setw(15) << "Agent" << std::setw(6) << "Games"
       << std::setw(6) << "Wins" << std::setw(6) << "Loss" << std::setw(6)
       << "Draw" << std::setw(8) << "Win%" << std::setw(8) << "AvgScore"
       << std::setw(6) << "TO" << std::setw(6) << "Crash" << std::endl;
  ...
  for (const auto &stats : rankedResults) {
    file << std::left << std::setw(15) << stats.agentName << std::setw(6)
         << stats.gamesPlayed << std::setw(6) << stats.wins << std::setw(6)
         << stats.losses << std::setw(6) << stats.draws << std::setw(8)
         << std::fixed << std::setprecision(1) << (stats.winRate * 100.0)
         << std::setw(8) << std::fixed << std::setprecision(1)
         << stats.averageScore << std::setw(6) << stats.timeouts << std::setw(6)
         << stats.crashes << std::endl;
  }
  ...
  for (size_t i = 0; i < gameResults_.size(); ++i) {
    ...
    file << "Game " << (i + 1) << ": " << result.blackAgent << " vs "
         << result.whiteAgent << "\n";
    file << "  Result: " << result.winner << " (" << result.blackScore << "-"
         << result.whiteScore << ")\n";
    file << "  Moves: " << result.moveCount
         << ", Duration: " << formatDuration(result.gameDuration) << "\n";
...
std::string
SimpleTournament::formatDuration(std::chrono::milliseconds duration) const {
  auto seconds = std::chrono::duration_cast<std::chrono::seconds>(duration);
  auto ms = duration % std::chrono::seconds(1);

  std::ostringstream oss;
  oss << seconds.count() << "." << std::setfill('0') << std::setw(3)
      << ms.count() << "s";
  return oss.str();
}
