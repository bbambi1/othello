...
AIAgentBase::getValidMoves(const Board &board, CellState player) const {
  return board.getValidMoves(player);
}

bool AIAgentBase::isValidMove(const Board &board, int row, int col,
...
std::unique_ptr<AIAgentBase> createAIAgent(const std::string &type,
